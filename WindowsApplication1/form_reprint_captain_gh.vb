...
Public Class form_reprint_captain_gh
    Dim con As New Mysql
    Public gh_id As String
    Private Sub form_reprint_captain_gh_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Login.OpenId = "0"
        Load_Product_list()
    ...
    Private Sub Load_Product_list()
        ...
        gh_id = gohome_list.TextBox2.Text
        ...
        & "pos_order_list.name_ord_en AS name_ord_en,pos_order_list.prs AS prs,pos_order_list.remark AS remark," _
        ...
        & " pos_order_list.status_open='gohome' and code_gohome ='" & gh_id & "' " _
        ...
        While dt.Read
            ...
            ElseIf dt.GetString("status_sd_captain").ToLower = "void" Or dt.GetString("status_sd_captain").ToLower = "voidp" Then
    ...
    Private Sub btn_drow_left_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_drow_left.Click
        ...
        ' End Try
    End Sub
    Public lang As String = Login.LangG
            ...
            If Is_itemCheck = True Then
                ...
                If result = DialogResult.Yes Then
                    If Login.switchLangPrintCaptain = 1 Then
                        switch_langprint.page = "form_reprint_captain_gh"
                            ...
                            If isSelected = True Then
                                ...
                                If DataGridView1.Rows(j).Cells(1).Value.ToString.ToLower = "yes" Or DataGridView1.Rows(j).Cells(1).Value.ToString.ToLower = "no" Then
                                    If array_print(t).ToString = opentable.Get_printer_subgroup(CStr(DataGridView1.Rows(j).Cells(12).Value.ToString())) Then
                                        ...
                                        & "'yes','no','gohome','" & remark & "'," _
                                        & "'" & dateNew.ToString("yyyy-MM-dd HH:mm:ss") & "','" & gh_id & "','" & DataGridView1.Rows(j).Cells(14).Value.ToString() & "','" & DataGridView1.Rows(j).Cells(13).Value.ToString() & "','" & array_idtemp(t).ToString & "');"
                                        If DataGridView1.Rows(j).Cells(1).Value.ToString.ToLower = "no" Then
                                            str_temp &= "UPDATE pos_order_list SET status_sd_captain='yes' Where status_sd_captain='no' and status_pay='no' and code_gohome='" & gh_id & "';"
                                        End If
                                    End If
                                End If
                            End If
                                    ...
                                    If DataGridView1.Rows(j).Cells(1).Value.ToString.ToLower = "void" Or DataGridView1.Rows(j).Cells(1).Value.ToString.ToLower = "voidp" Then
                                        If array_print_void(m).ToString = opentable.Get_printer_subgroup(DataGridView1.Rows(j).Cells(12).Value.ToString()) Then
                                            str_temp &= "INSERT INTO pos_temp_print (rf_id_prd,rf_id_con,name_ord,name_ord_en,amt,price,rf_id_table," _
                                               ...
                                               & "'" & dateNew.ToString("yyyy-MM-dd HH:mm:ss") & "','" & gh_id & "','" & DataGridView1.Rows(j).Cells(14).Value.ToString() & "','" & DataGridView1.Rows(j).Cells(13).Value.ToString() & "','" & array_idtemp_void(m).ToString & "');"
                                        End If
                                    End If
                                End If
                            Next
                    ...
                    If q = True Then
                        ...
                        For y As Integer = 0 To array_print.Count - 1
                            If array_print_onoff(y) <> "0" Or array_print_onoff(y) <> 0 Then
                                ...
                                Dim W As New worker
                                W.setSendCaptain(0, "gohome", index.getNameTable(Login.OpenId), dateNew.ToString("dd/MM/yyyy"), gh_id, Login.username, array_print(y).ToString, array_idtemp(y).ToString, array_sendcap(y).ToString, lang)
                        ...
                        If array_print_void.Count > 0 Then
                            For h As Integer = 0 To array_print_void.Count - 1
                                If array_print_onoff_void(h) <> "0" Or array_print_onoff_void(h) <> 0 Then
                                    Dim WorkerThread As Thread
                                    Dim W As New worker
                                    W.setSendCaptainCancel(0, "gohome", index.getNameTable(Login.OpenId), dateNew.ToString("dd/MM/yyyy"), gh_id, Login.username, array_print_void(h).ToString, array_idtemp_void(h).ToString)
                                    WorkerThread = New Thread(AddressOf W.Start)    'Create our Worker thread and tell it that when we start it it should call our Worker's Start() method
                                    WorkerThread.Start()
                                    WorkerThread.Join()
                                    If Login.alert_sendcaptain = 1 Then
    ...
    Private Sub item_up_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If DataGridView1.SelectedRows.Count > 0 Then
            If DataGridView1.CurrentRow.Index > 0 Then
                Dim temp1(19) As String
                Dim bcolor As Color
                Dim fcolor As Color

                For i As Integer = 0 To 19
                    temp1(i) = DataGridView1.SelectedRows.Item(0).Cells(i).Value
                Next
                bcolor = DataGridView1.SelectedRows.Item(0).DefaultCellStyle.BackColor
                fcolor = DataGridView1.SelectedRows.Item(0).DefaultCellStyle.ForeColor


                For i As Integer = 0 To 19
                    DataGridView1.SelectedRows.Item(0).Cells(i).Value = DataGridView1.Rows.Item(DataGridView1.SelectedRows.Item(0).Index - 1).Cells(i).Value
                Next

                DataGridView1.SelectedRows.Item(0).DefaultCellStyle.BackColor = DataGridView1.Rows.Item(DataGridView1.SelectedRows.Item(0).Index - 1).DefaultCellStyle.BackColor
                DataGridView1.SelectedRows.Item(0).DefaultCellStyle.ForeColor = DataGridView1.Rows.Item(DataGridView1.SelectedRows.Item(0).Index - 1).DefaultCellStyle.ForeColor

                For i As Integer = 0 To 19
                    DataGridView1.Rows.Item(DataGridView1.CurrentRow.Index - 1).Cells(i).Value = temp1(i)
                Next
                DataGridView1.Rows.Item(DataGridView1.SelectedRows.Item(0).Index - 1).DefaultCellStyle.BackColor = bcolor
                DataGridView1.Rows.Item(DataGridView1.SelectedRows.Item(0).Index - 1).DefaultCellStyle.ForeColor = fcolor

                'DataGridView1.Rows.Item(DataGridView1.SelectedRows.Item(0).Index - 1).Selected = True
                DataGridView1.CurrentCell = DataGridView1(0, DataGridView1.SelectedRows.Item(0).Index - 1)
                Update_IndexOrder()
            End If
        Else
    ...
    Private Sub item_drown_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        If DataGridView1.RowCount > 0 Then
            If DataGridView1.CurrentRow.Index < (DataGridView1.RowCount - 1) Then
                ...
                For i As Integer = 0 To 19
                    DataGridView1.SelectedRows.Item(0).Cells(i).Value = DataGridView1.Rows.Item(DataGridView1.CurrentRow.Index + 1).Cells(i).Value
                Next

                DataGridView1.SelectedRows.Item(0).DefaultCellStyle.BackColor = DataGridView1.Rows.Item(DataGridView1.CurrentRow.Index + 1).DefaultCellStyle.BackColor
                DataGridView1.SelectedRows.Item(0).DefaultCellStyle.ForeColor = DataGridView1.Rows.Item(DataGridView1.CurrentRow.Index + 1).DefaultCellStyle.ForeColor

                For i As Integer = 0 To 19
                    DataGridView1.Rows.Item(DataGridView1.CurrentRow.Index + 1).Cells(i).Value = temp1(i)
                Next
                DataGridView1.Rows.Item(DataGridView1.CurrentRow.Index + 1).DefaultCellStyle.BackColor = bcolor
                DataGridView1.Rows.Item(DataGridView1.CurrentRow.Index + 1).DefaultCellStyle.ForeColor = fcolor

                'DataGridView1.Rows.Item(DataGridView1.CurrentRow.Index + 1).Selected = True
                DataGridView1.CurrentCell = DataGridView1(0, DataGridView1.SelectedRows.Item(0).Index + 1)
                Update_IndexOrder()
            End If
        Else
        ...
        End If
    End Sub
    Private Sub Update_IndexOrder()
        Dim str As String = ""
        For j As Integer = 0 To DataGridView1.RowCount - 1
            Dim rf_id_prd As String = DataGridView1.Rows(j).Cells(3).Value.ToString()
            Dim rf_id_con As String = DataGridView1.Rows(j).Cells(4).Value.ToString()
            Dim name_ord As String = DataGridView1.Rows(j).Cells(5).Value.ToString().Replace("'", "\'")
            str &= "UPDATE pos_order_list SET index_print='" & j & "' where rf_id_prd='" & rf_id_prd & "' and rf_id_con='" & rf_id_con & "' and name_ord='" & name_ord & "' and code_gohome='" & gh_id & "' and status_pay='no' and create_date LIKE '%" & Login.DateData.ToString("yyyy-MM-dd") & "%';"
        Next
        If str <> "" Then
            con.mysql_query(str)
        End If
