...
Public Class payment_subp_dialog
    Dim con As New Mysql
    Dim ac As New Admin_ClassMain
    Public qty_subp As Integer = 0
    Public sel_bill As String = ""
    Private Sub payment_subp_dialog_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing
        payment.s1()
    End Sub
    Private Sub payment_subp_dialog_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Create_Btn()
        RecalPrice_QTY()
    End Sub
    Private Sub Create_Btn()
        FlowLayoutPanel1.Controls.Clear()
        For i As Integer = 0 To payment.FlowLayoutPanel_Bill.Controls.Count - 1
            ...
            btn.Tag = payment.FlowLayoutPanel_Bill.Controls.Item(i).Tag.ToString
            btn.Text = payment.FlowLayoutPanel_Bill.Controls.Item(i).Text.ToString
    ...
    Private Sub Enter_Btn(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        ' MsgBox("สถานะบิลเก่า:" & strArr_selbill(1) & ",สถานะบิลที่เลือก:" & strArr(1))
        ' MsgBox("เลขที่บิลเก่า:" & strArr_selbill(0) & ",เลขที่บิลที่เลือก:" & btn_id_inv)
        If (strArr_selbill(1) <> strArr(1)) Or btn_id_inv <> strArr_selbill(0) Then
            Dim StringQuery As String = ""
            Dim qprs As MySqlDataReader = con.mysql_query("select prs,ref_id_join from pos_order_list where rf_id_table='" & Login.OpenId & "' LIMIT 1;")
            qprs.Read()
            Dim prs As String = qprs.GetString("prs")
            '========================== insert new bill ============================================'
            '=== check ว่าย้ายกลับมาบิลที่มีอยู๋แล้วหรือเปล่า
            Dim Is_havebill As Boolean = False
            Dim invoice As MySqlDataReader = con.mysql_query("select rf_id_invoice from pos_order_list where status_pay='no' and status_open='ontb' and (rf_id_table='" & Login.OpenId & "' or ref_id_join='" & Login.OpenId & "') and rf_id_invoice='" & btn_id_inv & "' GROUP BY rf_id_invoice;")
            While invoice.Read()
            ...
            Next
            ' MsgBox(Is_havebill & "บิลที่เลือกอันใหม่" & btn_id_inv)
            Dim id As String = ""
            ' MsgBox(Is_havebill)
            ...
            For j As Integer = 0 To ListView1.Items.Count - 1
                ...
                Dim rf_id_join As Integer = ListView1.Items(j).SubItems(28).Text.Trim
                Dim prd_t_id As Integer = ListView1.Items(j).SubItems(29).Text.Trim
                Dim prd_t_en As String = ListView1.Items(j).SubItems(30).Text.Trim
                Dim prd_t_th As String = ListView1.Items(j).SubItems(31).Text.Trim
                ...
                Else
                    ...
                    & "'" & prs & "','" & id & "','" & rf_id_table & "','" & rf_id_join & "','" & status_sd_captain & "'," _
            ...
            StringQuery &= "DELETE FROM pos_order_list WHERE (rf_id_table='" & Login.OpenId & "' or ref_id_join='" & Login.OpenId & "' ) and rf_id_invoice='" & strArr_selbill(0) & "' and status_pay='no' and status_open='ontb';" '=== delete order old ==='
            Dim qty_new As Integer = 0
            Dim price_all_new As Double = 0.0
            For r As Integer = 0 To payment.ListView1.Items.Count - 1 '=== loop insert order new but invoice old ==='
                Dim status_sd_captain As String = payment.ListView1.Items(r).SubItems(0).Text
                Dim rf_id_prd As String = payment.ListView1.Items(r).SubItems(2).Text
                Dim rf_id_con As String = payment.ListView1.Items(r).SubItems(3).Text
                Dim name_ord As String = payment.ListView1.Items(r).SubItems(4).Text.Replace("'", "\'")
                Dim name_ord_en As String = payment.ListView1.Items(r).SubItems(5).Text.Replace("'", "\'")
                Dim qty As Integer = CInt(payment.ListView1.Items(r).SubItems(8).Text)
                Dim price As Double = CDbl(payment.ListView1.Items(r).SubItems(11).Text)
                Dim remark As String = payment.ListView1.Items(r).SubItems(16).Text.Replace("'", "\'")
                Dim id_cat As String = payment.ListView1.Items(r).SubItems(17).Text.Trim
                Dim id_subcatprd As String = payment.ListView1.Items(r).SubItems(18).Text.Trim
                Dim name_cat_en As String = payment.ListView1.Items(r).SubItems(19).Text.Trim
                Dim name_cat_th As String = payment.ListView1.Items(r).SubItems(20).Text.Trim
                Dim name_subcat_en As String = payment.ListView1.Items(r).SubItems(21).Text.Trim
                Dim name_subcat_th As String = payment.ListView1.Items(r).SubItems(22).Text.Trim
                Dim ord_vat As Integer = CInt(payment.ListView1.Items(r).SubItems(23).Text.Trim)
                Dim ord_vat_st As String = payment.ListView1.Items(r).SubItems(24).Text.Trim
                Dim rf_id_table As Integer = payment.ListView1.Items(r).SubItems(25).Text.Trim
                Dim ref_id_join As Integer = payment.ListView1.Items(r).SubItems(26).Text.Trim
                Dim prd_type_dis_id As Integer = payment.ListView1.Items(r).SubItems(28).Text.Trim
                Dim prd_type_dis_en As String = payment.ListView1.Items(r).SubItems(29).Text.Trim
                Dim prd_type_dis_th As String = payment.ListView1.Items(r).SubItems(30).Text.Trim
                If qty <> 0 Then
                    qty_new += qty
                    price_all_new += price
                    StringQuery &= "INSERT INTO pos_order_list (rf_id_prd,rf_id_con,name_ord,name_ord_en,amt,price,prs,rf_id_invoice," _
                    ...
                    & "ref_catsubprd,name_th_cat,name_en_cat,name_th_catsubprd,name_en_catsubprd,ord_vat,ord_vat_st,prd_type_dis_id,prd_type_dis_en,prd_type_dis_th) " _
                    & " VALUES('" & rf_id_prd & "','" & rf_id_con & "','" & name_ord & "','" & name_ord_en & "','" & qty & "','" & price & "'," _
                    & "'" & prs & "','" & strArr_selbill(0) & "','" & rf_id_table & "','" & ref_id_join & "','" & status_sd_captain & "','ontb','no'," _
                    & "'" & remark & "','" & Login.DateData.ToString("yyyy-MM-dd") & Date.Now.ToString(" HH:mm:ss") & "','" & Login.username & "'," _
                    & "'0','" & id_cat & "','" & id_subcatprd & "','" & name_cat_th & "'," _
                    & "'" & name_cat_en & "','" & name_subcat_th & "','" & name_subcat_en & "','" & ord_vat & "','" & ord_vat_st & "','" & prd_type_dis_id & "','" & prd_type_dis_en & "','" & prd_type_dis_th & "');"
                End If
            Next
            'MsgBox(StringQuery)
            Dim discount_des As String = ""
            Dim discount_sum As Double = 0.0
            If payment.Label_ShowAllDisType.Tag.ToString = "%" Then
                discount_des = payment.Label_ShowAllDisType.Tag.ToString
                discount_sum = FormatNumber(((CDbl(payment.txt_priceall.Text) + CDbl(payment.txt_sum_vat.Text) - 0) * CDbl(payment.txt_dis_all.Text)) / 100, 2)
            Else
                discount_des = payment.Label_ShowAllDisType.Tag.ToString
                discount_sum = FormatNumber(CDbl(payment.txt_dis_all.Text), 2)
            End If
            StringQuery &= "UPDATE pos_invoice_temp SET qty='" & qty_new & "',price_all='" & price_all_new & "',update_by='" & Login.username & "'," _
                & "vat='" & CDbl(payment.txt_sum_vat.Text) & "',price_exclu_vat='" & CDbl(payment.txt_priceall.Text) & "',discount='" & CDbl(payment.txt_dis_all.Text) & "'," _
            ...
            Dim c As Integer = con.mysql_num_rows(con.mysql_query("select * From pos_order_list where rf_id_table='" & Login.OpenId & "' and rf_id_invoice='" & strArr_selbill(0) & "' and status_pay='no' and status_open='ontb';"))
            If c <= 0 Then
                StringQuery &= "DELETE FROM pos_order_list where (rf_id_table='" & Login.OpenId & "' or ref_id_join='" & Login.OpenId & "') and rf_id_invoice='" & strArr_selbill(0) & "' and status_pay='no' and status_open='ontb';"
    ...
    Private Sub Button6_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button6.Click, ListView1.DoubleClick
        ...
        If ListView1.Items(ListView1.SelectedIndices(0)).SubItems(2).Text = "0" Then
            ...
            ' MsgBox(CInt(ListView1.Items(ListView1.SelectedIndices(0)).SubItems(24).Text))
            If CInt(ListView1.Items(ListView1.SelectedIndices(0)).SubItems(24).Text) = 0 Then
                str_vat = "None(vat)"
                vats = 0
                price_new1 = FormatNumber(pprice * qty_subp, 2)
                price_new2 = FormatNumber(pprice * qty_up_payment, 2)
            ElseIf CInt(ListView1.Items(ListView1.SelectedIndices(0)).SubItems(24).Text) = 1 Then
                str_vat = "Inc(vat)"
                vats = pprice - ((pprice * 100) / CInt(ListView1.Items(ListView1.SelectedIndices(0)).SubItems(23).Text + 100))
                price_new1 = FormatNumber((price_new - vats) * qty_subp, 2)
                price_new2 = FormatNumber((price_new - vats) * qty_up_payment, 2)
            ElseIf CInt(ListView1.Items(ListView1.SelectedIndices(0)).SubItems(24).Text) = 2 Then
                str_vat = "Add(vat)"
                vats = pprice * (CInt(ListView1.Items(ListView1.SelectedIndices(0)).SubItems(23).Text) / 100)
        ...
        For i As Integer = (payment.ListView1.Items.Count - 1) To 0 Step -1
            If payment.ListView1.Items(i).Checked = True Then
                If ListView1.Items(ListView1.SelectedIndices(0)).SubItems(4).Text = payment.ListView1.Items(i).SubItems(4).Text Then
                    payment.ListView1.Items(i).SubItems(8).Text = qty_up_payment
                    payment.ListView1.Items(i).SubItems(9).Text = price_new2
                    payment.ListView1.Items(i).SubItems(10).Text = FormatNumber(vats * CInt(qty_up_payment), 2)
                    payment.ListView1.Items(i).SubItems(11).Text = FormatNumber(CDbl(price_new2 + (vats * CInt(qty_up_payment))), 2)
                    payment.ListView1.Items(i).SubItems(15).Text = FormatNumber(CDbl(price_new2 + (vats * CInt(qty_up_payment))), 2)
                End If
            End If
        Next
    ...
    Public Sub RecalPrice_QTY()
        ...
        For i As Integer = (payment.ListView1.Items.Count - 1) To 0 Step -1
            If payment.ListView1.Items(i).Checked = True Then
                For j As Integer = 0 To ListView1.Items.Count - 1
                    If ListView1.Items(j).SubItems(4).Text = payment.ListView1.Items(i).SubItems(4).Text Then
                        payment.ListView1.Items(i).SubItems(8).Text = "0"
                        payment.ListView1.Items(i).SubItems(9).Text = "0"
                        payment.ListView1.Items(i).SubItems(10).Text = FormatNumber("0", 2)
                        payment.ListView1.Items(i).SubItems(11).Text = FormatNumber("0", 2)
                        payment.ListView1.Items(i).SubItems(15).Text = FormatNumber("0", 2)
                    End If
                Next
            End If
        Next
