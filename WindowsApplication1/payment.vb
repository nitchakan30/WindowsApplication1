...
Public Class payment
    ...
    Private Sub payment_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        End If
        opentable.ShowCover(Login.OpenId, txt_show_adult, txt_show_child, txt_show_cover, txt_room, txt_remark)
    End Sub
    ...
    Private Sub LoadSMType()
        ...
        Dim res_m As MySqlDataReader = con.mysql_query("SELECT * FROM pos_payment_type ORDER BY id ASC")
        Dim x As Integer = 8
        Dim y As Integer = 5
        ...
        While res_m.Read()
            If CInt(res_m.GetString("pay_active")) = 1 Then
                ...
                btn_m.Name = res_m.GetString("id")
                    ...
                    If CInt(res_m.GetString("id")) = rf_payment_type Then
    ...
    Private Sub Change_color(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        ElseIf rf_payment_type = 2 Then
            ...
            payment_inputtype.GroupBox_Coupon.Hide()
            payment_inputtype.GroupBox_Other.Hide()
            payment_inputtype.page = "payment"
        ...
        ElseIf rf_payment_type = 99 Then
            ...
            payment_inputbanktranfer.page = "payment"
    ...
    Public Sub EnterPay()
        ...
        For j As Integer = 0 To ListView1.Items.Count - 1
            disC_prd_sum += CDbl(ListView1.Items(j).SubItems(12).Text)
        Next
        ...
        If Label_ShowAllDisType.Tag.ToString = "บาท" Then
            ...
            disC = FormatNumber(CDbl(txt_dis_all.Text) - CDbl(disC_prd_sum), 2)
        Else
            Dim net1 As Double = 0
            For u As Integer = 0 To ListView1.Items.Count - 1
                net1 += CDbl(ListView1.Items(u).SubItems(15).Text)
            Next
            descount_des = Label_ShowAllDisType.Tag.ToString
            disC = FormatNumber(((net1 - discount_prd) * CDbl(txt_dis_all.Text)) / 100, 2)
        End If
        ...
        If selBill <> "" And selBill <> "0" Then  '========== เช้ึวว่ามี bill ที่เลือกหรือไม่ จาก Tabs No.1 ด้านล่าง ===='
            ...
            & "adult='" & txt_show_adult.Text & "',child='" & txt_show_child.Text & "',cover='" & txt_show_cover.Text & "'" _
            & " WHERE id='" & selBill & "';"
            ...
            str_query_invoice &= "INSERT INTO pos_invoice (id,namber_inv,number_pos,qty,price_all," _
                ...
                & "rf_id_card,money_recript,money_ton,close_rop_id_inv,close_rop_name_inv,adult,child,cover)" _
        ...
        ' MsgBox("บิลใหม่ตาราง inv:" & pos_invoice_id)
        Dim str_up As String = ""
        Dim prs As String = GetPrsInOrderList(selBill)
        For y As Integer = 0 To ListView1.Items.Count - 1
            ...
            Dim rf_id_table As String = ListView1.Items(y).SubItems(25).Text
            Dim ref_id_join As String = ListView1.Items(y).SubItems(26).Text
            'If ListView1.Items(y).SubItems(0).Text.ToLower <> "void" And ListView1.Items(y).SubItems(0).Text.ToLower <> "voidp" Then
            str_up &= "UPDATE pos_order_list SET prs='" & txt_show_cover.Text & "',order_dis_val='" & order_dis_val & "'," _
            & "order_dis_sum='" & order_dis_sum & "',order_dis_type='" & order_dis_type & "',rf_id_invoice='" & pos_invoice_id & "' " _
        ...
        End If
        Dim f As Integer = con.mysql_num_rows(con.mysql_query("select * from pos_order_list where (rf_id_table='" & Login.OpenId & "' or ref_id_join='" & Login.OpenId & "') and status_pay='no';"))
        If f <= 0 Then
            Dim up_tb As Boolean = con.mysql_query("UPDATE pos_table_system SET status='1',id_join_tb='0',invoice_number='0' WHERE (id='" & CInt(Login.OpenId) & "' or id_join_tb='" & Login.OpenId & "')")
            con.mysql_query("UPDATE pos_order_list_prs SET adult='0',child='0',cover='0',id_room='',room_number='',rf_id_invoice_temp='0',update_by='" & Login.username & "' WHERE rf_id_table='" & Login.OpenId & "'")
        End If
        InsertDataInvTo_CloseBillDetail(pos_invoice_id, CInt(Login.OpenId), 0)
        ...
        showPay.ihno = ihno
        showPay.Ref_Invoice = payment_gohome.GetNumberINV_POSINV(pos_invoice_id)
    ...
    Public Function Update_Orderlist_Discount(ByVal id_tb, ByVal idbill)
        ...
        Dim str_up As String = ""
        For y As Integer = 0 To ListView1.Items.Count - 1
            Dim order_dis_val As Double = CDbl(ListView1.Items(y).SubItems(11).Text)
            Dim order_dis_sum As Double = CDbl(ListView1.Items(y).SubItems(12).Text)
            Dim order_dis_type As String = ListView1.Items(y).SubItems(13).Text
            Dim rf_id_prd As String = ListView1.Items(y).SubItems(2).Text
            Dim rf_id_con As String = ListView1.Items(y).SubItems(3).Text
            Dim name_ord As String = ListView1.Items(y).SubItems(4).Text.Replace("'", "\'")
    ...
    Public Function InsertDataInvTo_CloseBillDetail(ByVal id_inv, ByVal rf_id_table, ByVal code_gohome)
        ...
        Dim ccode_gohome As String = ""
        If ListView1.Items.Count > 0 Then
            If CInt(rf_id_table) = 0 Then
                cstatus_open = "gohome"
                ccode_gohome = code_gohome
                rf_id_table = 0
            Else
                cstatus_open = "ontb"
                ccode_gohome = "0"
                rf_id_table = rf_id_table
            End If
            For i As Integer = 0 To ListView1.Items.Count - 1
                ...
                Dim prd_t_id As String = ListView1.Items.Item(i).SubItems(28).Text
                Dim prd_t_en As String = ListView1.Items.Item(i).SubItems(29).Text
                Dim prd_t_th As String = ListView1.Items.Item(i).SubItems(30).Text
                string1 &= "INSERT INTO pos_closebilldetail (cref_id_ord,crf_id_prd,crf_id_con,cname_ord,cname_ord_en,camt" _
                    ...
                    & "c_id_cat,c_id_subcat,c_name_cat_en,c_name_cat_th,c_name_subcat_en,c_name_subcat_th,cprd_type_dis_id,cprd_type_dis_en,cprd_type_dis_th) " _
                    & " VALUES('0','" & crf_id_prd & "','" & crf_id_con & "','" & cname_ord & "','" & cname_ord_en & "','" & camt & "'," _
                    & "'" & cprice & "','" & prs & "','" & rf_id_table & "','" & id_inv & "','" & c_vat & "'," _
                    & "'" & c_vat_st & "','" & c_vat_sum & "','" & c_discount & "','" & c_discount_sum & "','" & c_discount_type & "','" & c_amount & "','" & c_net_amount & "','" & cstatus_open & "'," _
                    & "'" & c_remark & "','" & cstatus & "','" & date_inv.ToString("yyyy-MM-dd HH:mm:ss") & "','" & ccode_gohome & "','" & tryNumber & "','" & Login.username & "','" & Login.Id_RopBill & "','" & Login.Str_Ropbill & "'" _
                    & ",'" & ref_id_cat & "','" & ref_id_subcat & "','" & name_cat_en & "','" & name_cat_th & "','" & name_subcat_en & "','" & name_subcat_th & "','" & prd_t_id & "','" & prd_t_en & "','" & prd_t_th & "');"
            Next
    ...
    Public id_bill_printtry As String = "0"
    Public type_bill_printtry As String = "novat"
    ...
    Private Sub print_rongbill_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles print_rongbill.Click
        ...
        For y As Integer = 0 To ListView1.Items.Count - 1
            ...
            If ListView1.Items(y).SubItems(0).Text.ToLower <> "void" And ListView1.Items(y).SubItems(0).Text.ToLower <> "voidp" Then
                ...
                & "order_dis_sum='" & order_dis_sum & "',order_dis_type='" & order_dis_type & "' where rf_id_prd='" & rf_id_prd & "' and rf_id_con='" & rf_id_con & "' and name_ord='" & name_ord.Replace("'", "\'") & "' " _
                & "and rf_id_invoice='" & id_bill & "' and status_pay='no' and status_open='ontb' and DAY(create_date)='" & Login.DateData.ToString("dd") & "' and MONTH(create_date)='" & Login.DateData.ToString("MM") & "' and YEAR(create_date)='" & Login.DateData.ToString("yyyy") & "' ;"
            End If
        Next
        ...
        str_y &= "UPDATE pos_invoice_temp SET qty='" & qty_prd & "',price_all='" & CDbl(txt_sum_total.Text) & "',create_by='" & Login.username & "'," _
          ...
          & "close_rop_id_inv='" & Login.Id_RopBill & "',close_rop_name_inv='" & Login.Str_Ropbill & "'," _
         & "adult='" & CInt(txt_show_adult.Text) & "',child='" & CInt(txt_show_child.Text) & "',cover='" & CInt(txt_show_cover.Text) & "'" _
          & " WHERE id='" & selBill & "';"
    ...
    Private Sub ListView1_DoubleClick(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ListView1.DoubleClick
        ...
        Else
            payment_discount.typedis = "bill"
            payment_discount.ShowDialog()
        End If  
    End Sub
    ...
    Private Sub ShowBill(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        payment_subp_dialog.sel_bill = btn.Tag.ToString
    ...
    Public Sub s1()
        ...
        Dim x As MySqlDataReader = con.mysql_query("SELECT * FROM pos_order_list WHERE status_pay='no' and (pos_order_list.rf_id_table='" & Login.OpenId & "' or pos_order_list.ref_id_join='" & Login.OpenId & "') GROUP BY rf_id_invoice ASC")
        Dim j As Integer = 1
        While x.Read()
            Dim btn As New Button
            btn.Name = "no" & j
    ...
    Private Sub s2(ByVal id_inv)
        ...
        If y > 0 Then
            ...
            Dim res_tb As MySqlDataReader = con.mysql_query("select * from view_order_list where create_date_orderlist LIKE '%" & Login.DateData.ToString("yyyy-MM-dd") & "%' and rf_id_invoice_order_list='" & id_inv & "' ORDER BY status_sd_captain DESC;")
            ...
            Dim snetamount As Double = 0.0
            Dim pprice1 As Double = 0.0
            Dim netamount As Double = 0.0
            Dim qty As Integer = 0
            Dim price_sum As Double = 0
            Dim price_novat As Double = 0
            While res_tb.Read
               ...
               If res_tb.GetString("rf_id_prd") = "0" Then
                ...
                qty = res_tb.GetInt32("samt")
                pprice1 = price_new1
                vsum = vats * qty
                snetamount = FormatNumber(CDbl(price_new1 + (vats * CInt(res_tb.GetInt32("samt")))), 2)
                netamount = FormatNumber((CDbl(price_new1 + (vats * CInt(res_tb.GetInt32("samt"))))) - CDbl(res_tb.GetString("order_dis_sum")), 2)

                itmx1.SubItems.Add(str_vat & " " & home1.vat_prd(res_tb.GetString("rf_id_prd")) & "%")
                itmx1.SubItems.Add(qty)
                itmx1.SubItems.Add(pprice1)
                itmx1.SubItems.Add(FormatNumber(vsum, 2))
                itmx1.SubItems.Add(snetamount)
                ...
                itmx1.SubItems.Add(GetNameTb(res_tb.GetString("rf_id_table")))
                itmx1.SubItems.Add(res_tb.GetString("prd_type_dis_id"))
                itmx1.SubItems.Add(res_tb.GetString("prd_type_dis_en"))
                itmx1.SubItems.Add(res_tb.GetString("prd_type_dis_th"))
                v += 1
            End While
    ...
    Private Sub btn_subpbill_del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_subpbill_del.Click
        If ListView1.Items.Count > 0 Then
            ...
            If result = DialogResult.Yes Then
                ...
                Dim qprs As MySqlDataReader = con.mysql_query("select prs,ref_id_join from pos_order_list where (rf_id_table='" & Login.OpenId & "' or ref_id_join='" & Login.OpenId & "') LIMIT 1;")
                qprs.Read()
                Dim prs As String = qprs.GetString("prs")
                ...
                For j As Integer = 0 To ListView1.Items.Count - 1
                    ...
                    Dim rf_id_table As Integer = ListView1.Items(j).SubItems(25).Text.Trim
                    Dim ref_id_join As Integer = ListView1.Items(j).SubItems(26).Text.Trim
                    Dim prd_t_id As Integer = ListView1.Items(j).SubItems(28).Text.Trim
                    Dim prd_t_en As String = ListView1.Items(j).SubItems(29).Text.Trim
                    Dim prd_t_th As String = ListView1.Items(j).SubItems(30).Text.Trim
                    Dim check_is_prd As Integer = con.mysql_num_rows(con.mysql_query("select * from pos_order_list " _
                        & "where rf_id_prd='" & rf_id_prd & "' and rf_id_con='" & rf_id_con & "' and name_ord='" & name_ord & "' and " _
                        & "rf_id_table='" & rf_id_table & "' and status_open='ontb' and rf_id_invoice='" & id & "' and status_pay='no';"))
                    ...
                    Else
                        ...
                        & "rf_id_table,ref_id_join,status_sd_captain,status_open,status_pay,remark,create_date,create_by,code_gohome,ref_cat_id," _
                        ...
                        & "'" & prs & "','" & id & "','" & rf_id_table & "','" & ref_id_join & "','" & status_sd_captain & "'," _
                        & "'ontb','no','" & remark & "','" & Login.DateData.ToString("yyyy-MM-dd") & Date.Now.ToString(" HH:mm:ss") & "','" & Login.username & "'," _
                        & "'0','" & id_cat & "','" & id_subcatprd & "','" & name_cat_th & "'," _
                        & "'" & name_cat_en & "','" & name_subcat_th & "','" & name_subcat_en & "','" & ord_vat & "','" & ord_vat_st & "','" & prd_t_id & "','" & prd_t_en & "','" & prd_t_th & "');"
                    End If
                Next
                StringQuery &= "DELETE FROM pos_order_list WHERE (rf_id_table='" & Login.OpenId & "' or ref_id_join='" & Login.OpenId & "') and rf_id_invoice='" & selBill & "' and status_pay='no' and status_open='ontb';" '=== delete order old ==='
    ...
    Private Sub btn_move_item_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_move_item.Click
        ...
        If t = True Then
            Dim itmx1 As New ListViewItem
            payment_subp_dialog.ListView1.Items.Clear()
            For i As Integer = (ListView1.Items.Count - 1) To 0 Step -1
                If ListView1.Items(i).Checked = True Then
                    If ListView1.Items(i).SubItems(0).Text <> "void" Then
                        itmx1 = payment_subp_dialog.ListView1.Items.Add(ListView1.Items(i).SubItems(0).Text, i)
                        ...
                        itmx1.SubItems.Add(ListView1.Items(i).SubItems(28).Text)
                        itmx1.SubItems.Add(ListView1.Items(i).SubItems(29).Text)
                        itmx1.SubItems.Add(ListView1.Items(i).SubItems(30).Text)
                    Else
                ...
                End If
            Next
            payment_subp_dialog.ShowDialog()
            payment_subp_dialog.RecalPrice_QTY()
        End If
    ...
    End Sub
    Private Sub txt_show_adult_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_show_adult.Click
        keyborad_number.text1 = "txt_adult_payment"
        keyborad_number.ShowDialog()
        opentable.AddCoverTable(txt_show_adult.Text, txt_show_child.Text, txt_show_cover.Text, "", "", Login.OpenId, "0")
    End Sub

    Private Sub txt_show_child_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txt_show_child.Click
        keyborad_number.text1 = "txt_child_payment"
        keyborad_number.ShowDialog()
        opentable.AddCoverTable(txt_show_adult.Text, txt_show_child.Text, txt_show_cover.Text, "", "", Login.OpenId, "0")
    End Sub

    Private Sub btn_disc_food_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_disc_food.Click
        payment_discount.typedis = "Food"
        payment_discount.ShowDialog()
    End Sub

    Private Sub btn_dis_bev_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_dis_bev.Click
        payment_discount.typedis = "Bev"
        payment_discount.ShowDialog()
    End Sub

    Private Sub btn_dis_other_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btn_dis_other.Click
        payment_discount.typedis = "Other"
        payment_discount.ShowDialog()
    End Sub
